...
To simulate the worst case, we generate 1000 traffic matrices for every topoloy with margin attribute $w$. Figure 5 shows
...
In Figure 3 (a), we can only observe five lines because there are two overlapping lines, which means that when removing 
links the worst case for the robust routing does not change. Maybe the random traffic matrices is not bad enough, or the 
removed link really do not affect the MLUR. And from Figure 3, we see that the OPRE is not always be achived for most
traffic matrices, the common value is much less than OPRE. 


Comparing two ways in first phase of REAR algorithm, in Abilene and Geant ERLU is always better than AC, furthermore in
some time, although more links are removed from topology the result of ERLU is still better. For most TMs, after switching 
off three links the OPRE is 1.2 and 1.6 respectively in Abilene and Geant. However, the ERLU is less optimal than ERLU in 
Cernet2, when we investigate the Cernet2 topology and find the crucial reason is the link between vertex Hangzhou and 
vertex Xiamen, which is least important in the meaning of algebraic connectivity rather ERLU.
...
robust for. Once the $w$ limit extremely, our robust routing is said without knowledge of traffic matrix. Figure 5 shows the 
OPRE increased as $w$ increased. It is obvious because when the $w$ is greater, the traffic matrix is random in more wider
range, and our OPRE may achieve worse case with more probability.


And we find when the margin increases from 1.5 to 2.0, the OPER increases much less, particularly in the begining of removing,
the difference almost can not be observed. It is a good phenomena for us, the variation of margin, i.e. the range of traffic
matrices affect little on the OPRE when the energy conservation is limited within 12\% in Geant.
...
We can observe that, ERLU is always shorter than AC, althoungh in Geant, the worst case is 36\% than the shortest path when margin 
