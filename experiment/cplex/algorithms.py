...
def ksp_yen(graph, node_start, node_end, max_k=2):
    ...
    for k in range(1, max_k):
        for i in range(0, len(A[-1]['path']) - 1):
            ...
            distance_spur = 0
            visited = []
            for le in range(len(path_root) - 1):
                if distance_spur < graph[path_root[le]][path_root[le+1]]:
                    distance_spur = graph[path_root[le]][path_root[le+1]]
                visited.append(path_root[le])
            ...
            path_spur = dijkstra(graph, node_spur, node_end, visited)
            
            if path_spur['path']:
                path_total = path_root[:-1] + path_spur['path']

                dist_total = 0
                if distance_spur > path_spur['cost']:
                    dist_total = distance_spur
                else:
                    dist_total = path_spur['cost']
