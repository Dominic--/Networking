...
by David pplegate[1]; and adjust routing in details according to the links we switched off in the final topology.
...
\begin{table}[!th]
	...
	\end{tabular}
\end{table}
where the $cap(l)$ isthe capacity of link $l$; 
...
The routing we get indicate how to arrive destination node from source node for every OD pair in the origin topology.
...
instead of the direct link $i->j$, then we just replace the routhing $s \rightarrow i \rightarrow j \rightarrow d$ with
$s \rightarrow i \rightarrow \rightarrow k \rightarrow j \rightarrow d$.
...
$\quad\qquad\quad$ $S$ which is the switch-off links set generated by Phase One;\\
...
$\:\:$4:\quad\ $ls \leftarrow node of l$, $ld \leftarrow the other node of l$;\\
$\:\:$5:\quad\ $Replace[ls, ld] \leftarrow find path form ls to ld on G^*$;\\
$\:\:$6:\ $Routing \leftarrow run LP in CPLEX, fetch routing by parsing result$;\\
$\:\:$7:\ \textbf{for} {each r in Routing}\\
$\:\:$8:\quad\ \textbf{if} $l \in S in r $\\
$\:\:$9:\quad\ \quad\ $s \leftarrow source of r$, $d \leftarrow destination of r$;\\
$\:\:$10:\quad\ \quad\ $r \leftarrow r[s, ls] + Replace[ls,ld] + r[ld,d]$;\\
$\:\:$11:\ \textbf{return} Routing;\\
\hline
\end{tabular}
\end{table}
