...
Internet is playing a more and more important role in our daily life, as an huge and indispensable communication system
it has considerable share in energy consumption. Network components such as routers and switchs contribute to the prominent
consumption part on account of inefficient energy management solutions, which makes the devices always working in full mode 
regardless of traffic demands. In particular, even there are no traffic on the link between two routers or switches, 
the line cards related to the link are still working. Line cards are the main part of energy consuming in routers or switches 
which is researched in previous work. For the energy consideration, we should obtain link-level solutions that
switches the link (line cards) to sleep mode if possible.
...
We simple take an power model from \cite{networking:greente} showed in Table \Rmnum{1}, and defined the difference of power consumption 
